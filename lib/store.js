  ...
  constructor(initialState: InitialState, calendar: ApocCalendar) {
    ...
    if (this._hasRef()) {
      ...
      const date = addMonths(
        new Date(ref.store.year, ref.store.month),
        ref.store.refs.length
      );
      this.set({
      ...
      // TODO: マルチなカレンダーでも割り切れる数になるように調整する
      // const diff: number = differenceInCalendarYears(date.max, date.min) + 1;
      const diff: number = differenceInCalendarYears(date.max, date.min) + 2;
      const years = (range(diff): number[]).map((amount, i) => {
        const year = addYears(date.min, amount);
        let start: Date = year;
        let end: Date = date.max;
        ...
        return new Year(start, end, date.max, date.min);
      });
    ...
    this.compute(
      ...
      (years, currentYear, currentMonth) => {
        ...
        const {day: firstDay} = dates[0];
        const {day: lastDay} = dates[dates.length - 1];
        for (let i = 0; i < firstDay; i++) {
          dates.unshift(new DatePad());
        }
        /** fills late a month with PadDate */
        for (let i = 6; i > lastDay; i--) {
          dates.push(new DatePad());
        }

        if (
          isBefore(this.minDate, addSeconds(this.currentDate, 1)) &&
          isAfter(
            subSeconds(this.currentDate, 1),
            addMonths(this.maxDate, this._getRefsSize() + 1)
          )
        ) {
  ...
  */
  ...
  setPrevMonth(fromRef = false, provisionalPrevDate: Date | undefined) {
    ...
    if (typeof provisionalPrevDate === 'undefined') {
      ...
      provisionalPrevDate = subMonths(
        new Date(firstCalendar.store.year, firstCalendar.store.month),
        size + 1
      );
    }

    if (isBefore(provisionalPrevDate, subMonths(this.minDate, size + 1))) {
  ...
  setNextMonth(fromRef = false, provisionalNextDate: Date | undefined) {
    ...
    if (typeof provisionalNextDate === 'undefined') {
      ...
      provisionalNextDate = addMonths(
        new Date(lastCalendar.store.year, lastCalendar.store.month),
        size + 1
      );
    }

    if (isAfter(provisionalNextDate, addMonths(this.maxDate, size + 1))) {
