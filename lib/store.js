  ...
  constructor(initialState: InitialState, calendar: ApocCalendar) {
    ...
    this.compute(
      ...
      (years, currentYear, currentMonth) => {
        ...
        if (isBefore(this.minDate, addSeconds(this.currentDate, 1)) && isAfter(subSeconds(this.currentDate, 1), this.maxDate)) {
  ...
  */
  ...
  setNextMonth(fromRef = false, provisionalNextDate: Date | undefined) {
    ...
    if (typeof provisionalNextDate === 'undefined') {
      ...
      provisionalNextDate = subMonths(new Date(lastCalendar.store.year, lastCalendar.store.month), size + 1);
    }
