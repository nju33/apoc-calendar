...
export default class CalendarStore extends Store {
  ...
  setData(newestData) {
    ...
    //
    setTimeout(() => {
      const {year, month, currentDates} = this.get();
    ...
    Object.keys(newestData).forEach(year => {
      newestData[year].forEach((month, monthIndex) => {
        month.forEach((date, dateIndex) => {
          const path = `${year}[${monthIndex}][${dateIndex}]`;

          const got = get(data, path, false);
          if (!got) {
            return;
          }

          set(data, `${path}.selected`, got.selected);
        });
      });
    });
    this.set({
  ...
  selectDay(day) {
    const {year, month, data, currentDates, pad} = this.get();
    // const currentDates = this.get('currentDates');
